...
contract InventoryPayment {
    ...
    function addSupplier(
        ...
        suppliers[supplierCount] = Supplier(
            ...
            _supplierName,
            _supplierNumber,
            SupplierBank(_bankName, _sortCode, _bankAccountNumber,0)
        );
    ...
    function receiveOrder(uint256 _orderNo, OrderDetail[] memory _receivedProducts) external {
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            if (orders[i].orderNo == _orderNo && !compareStrings(orders[i].orderStatus, "received") && !compareStrings(orders[i].orderStatus, "paid") ) {
                uint256 totalAmount = 0;
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            if (orders[i].orderNo == _orderNo && compareStrings(orders[i].orderStatus, "received") && !compareStrings(orders[i].orderStatus, "paid")) {
                ...
                Supplier storage supplier = suppliers[supplierId];

                supplier.supplierBank.bankAmount= supplier.supplierBank.bankAmount + totalAmount;
                suppliers[supplierId] = supplier;
                //Here we implement the payment integration in future such as paypal and stripe.
